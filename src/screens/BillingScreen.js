...
export default function BillingScreen({ route, navigation }) {
  ...
  // Dynamic order data from route params
  const orderData = route?.params?.orderData || {
    id: 'ORD-101',
    table: 'T-02',
    customer: 'Julien',
    items: [
      { name: 'Truffle Wagyu Ribeye', price: 6800 },
      { name: 'Yuzu Basil Smash', price: 2800 },
    ],
  };
  ...
  // Safe Discount Input Validation (Prevents > 100% and negative values)
  ...
  const discountVal = Math.min(Math.max(parseFloat(discountPercent) || 0, 0), 100);
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            <Text style={styles.cardHeaderTitle}>Customer: {orderData.customer || 'Walk-in Guest'}</Text>
          ...
          <View style={styles.card}>
            ...
            {selectedMethod === 'Cash' && (
              <View style={{ marginTop: 14 }}>
                <Text style={styles.calcLabel}>Cash Received (Rs.)</Text>
                <TextInput
                  style={styles.cashInput}
                  keyboardType="numeric"
                  value={cashTendered}
                  onChangeText={(val) => setCashTendered(val.replace(/[^0-9.]/g, ''))}
          ...
          {/* Settle Action */}
          <TouchableOpacity
            style={[styles.settleBtn, isProcessing && { opacity: 0.6 }]}
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <Text style={styles.settleBtnText}>
                {isProcessing ? 'Processing...' : 'Complete & Clear Table'}
              </Text>
              {!isProcessing && (
