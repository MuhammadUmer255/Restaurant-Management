...
import { Search, Building2, Users, Receipt, ShieldCheck, Activity, PlusCircle, Moon, Sun, X, ArrowRight, } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
export const CommandPaletteModal = ({ isOpen, onClose, tenants, onSelectTenant, onNavigate, onOpenOnboarding, onOpenProvisioning, toggleDarkMode, isDarkMode, }) => {
    ...
    const filteredTenants = query.trim()
        ? tenants.filter((t) => t.tradeName.toLowerCase().includes(query.toLowerCase()) ||
            ...
            t.primaryContact.email.toLowerCase().includes(query.toLowerCase())).slice(0, 5)
        : tenants.slice(0, 4);
    return (<AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4 sm:px-6">
        {/* Backdrop */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"/>

        {/* Modal Dialog */}
        <motion.div initial={{ opacity: 0, scale: 0.96, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: -10 }} className="relative w-full max-w-2xl rounded-2xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden z-10">
          {/* Search Header */}
          <div className="flex items-center px-4 py-3.5 border-b border-slate-200 dark:border-slate-800 gap-3">
            <Search className="w-5 h-5 text-slate-400"/>
            <input ref={inputRef} type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Type a command, search restaurants by name, ID, or contact..." className="flex-1 bg-transparent text-slate-900 dark:text-slate-100 placeholder-slate-400 text-sm focus:outline-none"/>
          ...
          <div className="max-h-96 overflow-y-auto p-2 divide-y divide-slate-100 dark:divide-slate-800/60">
              ...
              {onOpenProvisioning && (
                <button onClick={() => {
                  ...
                  <span className="flex items-center gap-2.5">
                    ...
                    <span>Provision Restaurant Business in Supabase Table 'tenants'</span>
                  </span>
            ...
            onClose();
                ...
                <span className="flex items-center gap-2.5">
                  ...
                  <span>Launch Full Multi-Step Onboarding Wizard</span>
                </span>
            ...
            <div className="py-2">
              ...
              {filteredTenants.length === 0 ? (<div className="px-3 py-6 text-center text-xs text-slate-400">
                  No restaurants matching your query.
                </div>) : (filteredTenants.map((t) => (<button key={t.id} onClick={() => {
                onSelectTenant(t);
                onClose();
