...
import { Search, MoreVertical, Sliders, ShieldCheck, Pause, Play, Trash2, Download, PlusCircle, Building2, Copy, Check, Eye, CheckSquare, Square, ArrowUpDown, } from 'lucide-react';
import { StatusBadge, PlanBadge } from '../common/Badge';
import { DispatchNoticeModal } from './DispatchNoticeModal';
import { useToast } from '../common/Toast';
export const TenantManagement = ({ tenants, onSelectTenant, onImpersonate, onEditLimits, onToggleStatus, onRevokeAccess, onOpenOnboarding, onOpenProvisioning, onSendBatchNotice, }) => {
    ...
    const filteredTenants = useMemo(() => {
        return tenants
            .filter((t) => {
            const matchesSearch = t.tradeName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const handleExportCSV = () => {
        ...
        const rows = filteredTenants
            .map((t) => `"${t.id}","${t.tradeName}","${t.businessName}","${t.primaryContact.name}","${t.primaryContact.email}","${t.planTier}",${t.mrr},"${t.status}","${t.region}","${t.dateJoined}"`)
            .join('\n');
    ...
    return (<div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleExportCSV} className="px-2.5 py-1.5 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-xs font-medium flex items-center gap-1.5 transition-colors">
            <Download className="w-3.5 h-3.5"/>
            <span>Export CSV</span>
          </button>
          {onOpenProvisioning && (
            <button onClick={onOpenProvisioning} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs transition-colors" title="Register restaurant in Supabase table 'tenants' (name, contact_email, subscription_tier)">
              <Building2 className="w-3.5 h-3.5"/>
              <span>Provision Tenant</span>
            </button>
          )}
          <button onClick={onOpenOnboarding} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-zinc-900 dark:bg-zinc-100 hover:bg-zinc-800 dark:hover:bg-white text-white dark:text-zinc-900 text-xs font-medium transition-colors">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-lg p-3 border border-zinc-200 dark:border-zinc-800 space-y-2.5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search by restaurant name, ID, contact, or legal entity..." value={searchQuery} onChange={(e) => {
        ...
        {/* Bulk Actions Bar */}
        {selectedTenantIds.length > 0 && (<div className="p-2 rounded-md bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-between text-xs">
            <span className="font-medium text-zinc-800 dark:text-zinc-200">
              {selectedTenantIds.length} restaurant{selectedTenantIds.length > 1 ? 's' : ''} selected
            </span>
            <div className="flex items-center gap-2">
              <button onClick={() => setIsDispatchNoticeOpen(true)} className="px-2.5 py-1 rounded border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 font-medium">
                Notice
              </button>
              <button onClick={handleExportSelectedCSV} className="px-2.5 py-1 rounded bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 font-medium">
                Export
              </button>
              <button onClick={() => setSelectedTenantIds([])} className="text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 text-[11px]">
                Clear
              </button>
            </div>
          </div>)}
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-zinc-600 dark:text-zinc-400">
            ...
            <thead className="bg-zinc-50 dark:bg-zinc-900 text-[11px] font-medium text-zinc-500 dark:text-zinc-400 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-10">
              <tr>
                <th className="py-2.5 pl-3.5 pr-2 w-8">
                  <button onClick={handleSelectAll} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200" aria-label="Select all tenants">
                    {selectedTenantIds.length === paginatedTenants.length && paginatedTenants.length > 0 ? (<CheckSquare className="w-3.5 h-3.5 text-zinc-900 dark:text-zinc-100"/>) : (<Square className="w-3.5 h-3.5"/>)}
                  </button>
                </th>
                <th className="py-2.5 px-3">ID</th>
                <th onClick={() => handleSort('tradeName')} className="py-2.5 px-3 cursor-pointer select-none hover:text-zinc-900 dark:hover:text-zinc-100">
                ...
                <th onClick={() => handleSort('mrr')} className="py-2.5 px-3 cursor-pointer select-none hover:text-zinc-900 dark:hover:text-zinc-100">
                  <div className="flex items-center gap-1">
                    <span>MRR</span>
                    <ArrowUpDown className="w-3 h-3 text-zinc-400"/>
                  </div>
                ...
                <th onClick={() => handleSort('dateJoined')} className="py-2.5 px-3 cursor-pointer select-none hover:text-zinc-900 dark:hover:text-zinc-100">
                  <div className="flex items-center gap-1">
                    <span>Joined</span>
                    <ArrowUpDown className="w-3 h-3 text-zinc-400"/>
                  </div>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
              {paginatedTenants.length === 0 ? (<tr>
                  <td colSpan={9} className="py-10 text-center text-zinc-400">
                    No restaurant tenants found.
                  </td>
                </tr>) : (paginatedTenants.map((tenant) => {
            const isSelected = selectedTenantIds.includes(tenant.id);
            const isMenuOpen = activeMenuTenantId === tenant.id;
            return (<tr key={tenant.id} onClick={() => onSelectTenant(tenant)} className={`cursor-pointer transition-colors ${isSelected
                    ? 'bg-zinc-100/60 dark:bg-zinc-800/40'
                    : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/30'}`}>
                      {/* Checkbox */}
                      <td className="py-2.5 pl-3.5 pr-2">
                        <button onClick={(e) => handleToggleSelectOne(tenant.id, e)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200">
                          {isSelected ? (<CheckSquare className="w-3.5 h-3.5 text-zinc-900 dark:text-zinc-100"/>) : (<Square className="w-3.5 h-3.5"/>)}
                        </button>
                      </td>
                      ...
                      <td className="py-2.5 px-3 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={(e) => handleCopyId(tenant.id, e)} className="p-0.5 rounded text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200" title="Copy ID">
                            {copiedTenantId === tenant.id ? (<Check className="w-3 h-3 text-emerald-500"/>) : (<Copy className="w-3 h-3"/>)}
                          </button>
                        </div>
                      </td>
                      ...
                      <td className="py-2.5 pr-3.5 pl-2 text-right relative">
                        <div onClick={(e) => e.stopPropagation()} className="inline-block">
                          <button onClick={() => setActiveMenuTenantId(isMenuOpen ? null : tenant.id)} className="p-1 rounded text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800" aria-label="Actions">
                            <MoreVertical className="w-3.5 h-3.5"/>
                          </button>

                          {/* Action Menu Popover */}
                          {isMenuOpen && (<div className="absolute right-3 top-8 w-44 rounded-md bg-white dark:bg-zinc-900 shadow-lg border border-zinc-200 dark:border-zinc-800 py-1 z-30 text-left text-xs">
                              <button onClick={() => {
        ...
        <div className="p-3 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            <button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} className="px-2.5 py-1 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 font-medium disabled:opacity-40 disabled:cursor-not-allowed hover:bg-zinc-50 text-[11px]">
              Previous
            </button>
            <div className="flex items-center gap-1">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (<button key={p} onClick={() => setCurrentPage(p)} className={`w-6 h-6 rounded text-[11px] font-medium ${currentPage === p
                ? 'bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 font-semibold'
                : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>
                  {p}
                </button>))}
            </div>
            <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="px-2.5 py-1 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 font-medium disabled:opacity-40 disabled:cursor-not-allowed hover:bg-zinc-50 text-[11px]">
              Next
            </button>
          </div>
        </div>
      ...
      <DispatchNoticeModal isOpen={isDispatchNoticeOpen} onClose={() => setIsDispatchNoticeOpen(false)} selectedTenantIds={selectedTenantIds} tenants={tenants} onDispatchNotice={(noticeData) => {
