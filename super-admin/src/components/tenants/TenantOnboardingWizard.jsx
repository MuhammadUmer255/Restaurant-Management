...
import { X, Check, ChevronRight, ChevronLeft, Building2, UserCheck, Sliders, CheckCircle2, Key, Copy, RefreshCw, Loader2, Terminal, Zap, MapPin, ExternalLink, } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { registerTenantInSupabase, registerRestaurantUserInSupabase, getSupabaseConfig, } from '../../lib/supabase';
import { LocationPickerMap } from '../maps/LocationPickerMap';
import { getGoogleMapsExternalUrl } from '../../lib/maps';
export const TenantOnboardingWizard = ({ isOpen, onClose, onTenantCreated, onUserCreated, }) => {
    ...
    const [posTerminalsCap, setPosTerminalsCap] = useState(6);
    ...
    return (<AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs">
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="w-full max-w-4xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] text-xs">
          {/* Wizard Header */}
          <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            ...
            <button onClick={onClose} disabled={isDeploying} className="p-1 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
              <X className="w-4 h-4"/>
            </button>
          </div>
          ...
          <div className="px-6 py-3 bg-slate-50 dark:bg-slate-950/60 border-b border-slate-100 dark:border-slate-800">
            ...
            return (<React.Fragment key={s.num}>
                    <div className="flex items-center gap-2">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-[11px] transition-colors ${isActive
                    ...
                    : isCompleted
                      ...
                      <span className={`font-semibold text-[11px] hidden sm:inline ${isActive
                    ...
                    {idx < steps.length - 1 && (<div className={`flex-1 h-0.5 mx-2 rounded ${currentStep > s.num
                        ? 'bg-emerald-500'
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            {currentStep === 1 && (<div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" placeholder="e.g. Haveli Heritage Dining Pvt Ltd" value={legalName} onChange={(e) => setLegalName(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500" required/>
                  </div>
                  <div className="space-y-1.5">
                    <label className="font-semibold text-slate-700 dark:text-slate-300">
                      Restaurant Brand / Trade Name (DBA) *
                    </label>
                    <input type="text" placeholder="e.g. Haveli Restaurant Lahore" value={tradeName} onChange={(e) => setTradeName(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500" required/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" placeholder="e.g. PK-NTN-7391024-3" value={regNumber} onChange={(e) => setRegNumber(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"/>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                ...
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 space-y-1.5">
                  ...
                  <input type="text" placeholder="e.g. resto_4920 or existing UUID in your project database" value={externalRestaurantId} onChange={(e) => setExternalRestaurantId(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-mono text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500"/>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <select value={region} onChange={(e) => setRegion(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select value={timezone} onChange={(e) => setTimezone(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select value={locale} onChange={(e) => setLocale(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            {currentStep === 2 && (<div className="space-y-3">
                ...
                <div className="h-[430px] rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xs">
                  <LocationPickerMap isModal={false} restaurantName={tradeName || 'New Restaurant'} initialLocation={restaurantLocation} onSaveLocation={(loc) => {
            ...
            {currentStep === 3 && (<div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" placeholder="e.g. Tariq Mehmood" value={ownerName} onChange={(e) => setOwnerName(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500" required/>
                  </div>
                  <div className="space-y-1.5">
                    <label className="font-semibold text-slate-700 dark:text-slate-300">
                      Owner Email Address (Admin Login) *
                    </label>
                    <input type="email" placeholder="owner@restaurant.pk" value={ownerEmail} onChange={(e) => setOwnerEmail(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500" required/>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="text" placeholder="+92 (300) 8414899" value={ownerPhone} onChange={(e) => setOwnerPhone(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"/>
                </div>
                ...
                <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={generateNewPassword} className="flex items-center gap-1 text-[11px] text-emerald-600 hover:text-emerald-700 font-medium">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={copyPasswordToClipboard} className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-1.5 font-medium">
                      {passwordCopied ? (<>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none text-slate-600 dark:text-slate-400">
                    <input type="checkbox" checked={sendInviteEmail} onChange={(e) => setSendInviteEmail(e.target.checked)} className="rounded text-emerald-600 focus:ring-emerald-500 h-4 w-4"/>
            ...
            {currentStep === 4 && (<div className="space-y-4">
                ...
                },
                {
            ...
            ].map((plan) => (<div key={plan.name} onClick={() => {
                    setSelectedPlan(plan.name);
                    if (plan.name === 'Starter')
                        setPosTerminalsCap(2);
                    if (plan.name === 'Professional')
                        setPosTerminalsCap(6);
                    if (plan.name === 'Enterprise')
                        setPosTerminalsCap(15);
                }} className={`p-3.5 rounded-xl border cursor-pointer transition-all ${selectedPlan === plan.name
                    ...
                    : 'border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'}`}>
                        <div className="flex items-center justify-between">
                          ...
                          {plan.recommended && (<span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-600 text-white uppercase">
                              Popular
                            </span>)}
                        </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <select value={trialDays} onChange={(e) => setTrialDays(Number(e.target.value))} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="number" min={1} max={50} value={posTerminalsCap} onChange={(e) => setPosTerminalsCap(Number(e.target.value))} className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"/>
                  </div>
                </div>
            ...
            ].map((flag) => (<label key={flag.key} className="p-2.5 rounded-xl border border-slate-200/70 dark:border-slate-800 flex items-center justify-between cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 select-none">
                        <span className="text-slate-700 dark:text-slate-300 font-medium">
                          {flag.label}
                        </span>
                        <input type="checkbox" checked={featureFlags[flag.key]} onChange={(e) => setFeatureFlags((prev) => ({
                    ...prev,
            ...
            <div className="p-6 rounded-2xl bg-slate-950 text-slate-200 space-y-4 font-mono">
                    ...
                    <div className="space-y-1.5 text-xs text-slate-400">
                      {deploymentLogs.map((log, i) => (<div key={i} className="flex items-center gap-2">
                    ...
                    <div className="p-4 rounded-xl bg-emerald-50/50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/50 space-y-3">
                      ...
                      <div className="p-3 rounded-lg bg-white dark:bg-slate-900 border border-emerald-200/60 dark:border-emerald-800/40 space-y-1.5">
                        <div className="flex items-center justify-between">
                          ...
                          <a href={getGoogleMapsExternalUrl(restaurantLocation.coordinates, tradeName)} target="_blank" rel="noopener noreferrer" className="text-[11px] text-emerald-600 hover:underline flex items-center gap-1">
          ...
          <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 flex items-center justify-between">
            {currentStep > 1 ? (<button type="button" disabled={isDeploying} onClick={() => setCurrentStep((prev) => (prev - 1))} className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 font-semibold flex items-center gap-1.5">
                <ChevronLeft className="w-4 h-4"/>
                <span>Back</span>
              </button>) : (<button type="button" onClick={onClose} className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800">
                Cancel
              </button>)}

            {currentStep < 5 ? (<button type="button" onClick={() => setCurrentStep((prev) => (prev + 1))} className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold flex items-center gap-1.5 shadow-xs">
                <span>Continue</span>
                <ChevronRight className="w-4 h-4"/>
              </button>) : (<button type="button" disabled={isDeploying} onClick={handleDeploy} className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold flex items-center gap-2 shadow-xs transition-colors">
                {isDeploying ? (<>
