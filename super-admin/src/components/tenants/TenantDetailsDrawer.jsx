...
import { X, Building2, Mail, ShieldCheck, Sliders, Pause, Play, Trash2, ExternalLink, MapPin, Layers, Monitor, Edit3, } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { StatusBadge } from '../common/Badge';
import { LocationPickerMap } from '../maps/LocationPickerMap';
import { getGoogleMapsExternalUrl } from '../../lib/maps';
export const TenantDetailsDrawer = ({ tenant, isOpen, onClose, onImpersonate, onEditLimits, onToggleStatus, onRevokeAccess, onToggleFeatureFlag, onUpdateLocation, onOpenKeyConfig, }) => {
    ...
    return (<AnimatePresence>
      <div className="fixed inset-0 z-50 flex justify-end bg-zinc-950/50 backdrop-blur-2xs">
        ...
        {/* Drawer panel */}
        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%', transition: { ease: 'easeInOut', duration: 0.2 } }} className="relative w-full max-w-lg bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 shadow-2xl flex flex-col h-full z-10 overflow-hidden text-xs">
          {/* Drawer Header */}
          <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-start justify-between gap-3">
            ...
            <button onClick={onClose} className="p-1 rounded text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800">
              <X className="w-4 h-4"/>
            </button>
          </div>
          ...
          <div className="px-4 py-2.5 bg-zinc-50 dark:bg-zinc-900/60 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-2">
            ...
            <button onClick={() => onEditLimits(tenant)} className="flex items-center gap-1 py-1.5 px-2.5 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-200 rounded-md font-medium transition-colors">
              <Sliders className="w-3 h-3"/>
              <span>Limits</span>
            </button>
            <button onClick={() => onToggleStatus(tenant)} className="p-1.5 rounded-md border border-zinc-200 dark:border-zinc-700 text-zinc-600 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors" title={tenant.status === 'Suspended' ? 'Resume Subscription' : 'Pause Subscription'}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setIsLocationModalOpen(true)} className="flex items-center gap-1 text-[11px] text-emerald-600 dark:text-emerald-400 hover:underline font-medium">
                ...
                {loc ? (<>
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        {loc.landmark && (<p className="text-[10px] text-zinc-500 mt-0.5">
                            Landmark: <span className="font-medium text-zinc-700 dark:text-zinc-300">{loc.landmark}</span>
                          </p>)}
                      </div>

                      <a href={getGoogleMapsExternalUrl(loc.coordinates, tenant.tradeName)} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-md border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 bg-white dark:bg-zinc-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 flex items-center gap-1 text-[10px] shrink-0" title="Open in Google Maps in new window">
                        <span>Open Maps</span>
                    ...
                    <button type="button" onClick={() => setIsLocationModalOpen(true)} className="px-3 py-1 bg-emerald-600 text-white rounded text-[11px] font-semibold hover:bg-emerald-700">
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <a href={`mailto:${tenant.primaryContact.email}`} className="text-zinc-700 dark:text-zinc-300 hover:underline font-medium">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => onEditLimits(tenant)} className="text-zinc-600 dark:text-zinc-400 hover:underline text-xs">
                  Adjust
                </button>
              </div>
              ...
              <div className="p-3 rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 space-y-2.5">
                ...
                <div>
                  ...
                  <div className="w-full h-1.5 rounded-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden">
                    <div className="h-full bg-zinc-900 dark:bg-zinc-100 rounded-full" style={{
            width: `${Math.min(100, (tenant.limits.posTerminals.current / tenant.limits.posTerminals.max) * 100)}%`,
                ...
                <div>
                  ...
                  <div className="w-full h-1.5 rounded-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden">
                    <div className="h-full bg-zinc-600 dark:bg-zinc-400 rounded-full" style={{
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(tenant.featureFlags).map(([flagKey, enabled]) => (<button key={flagKey} type="button" onClick={() => onToggleFeatureFlag?.(tenant, flagKey)} className="p-2 rounded-md border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center justify-between transition-colors text-left">
                    <span className="text-[11px] font-medium text-zinc-700 dark:text-zinc-300 capitalize">
                      {flagKey.replace('Enabled', '').replace(/([A-Z])/g, ' $1')}
                    </span>
                    <div className={`relative inline-flex h-3.5 w-7 shrink-0 cursor-pointer rounded-full border border-transparent transition-colors duration-200 ease-in-out ${enabled ? 'bg-zinc-900 dark:bg-zinc-100' : 'bg-zinc-300 dark:bg-zinc-700'}`}>
                      <span className={`pointer-events-none inline-block h-2.5 w-2.5 transform rounded-full ${enabled ? 'bg-white dark:bg-zinc-900 translate-x-3.5' : 'bg-white translate-x-0.5'} shadow-xs transition duration-200 ease-in-out mt-0.5`}/>
                    </div>
                  </button>))}
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-zinc-200 dark:border-zinc-800 space-y-2">
              ...
              <button onClick={() => onRevokeAccess(tenant)} className="w-full flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-md border border-rose-200 dark:border-rose-900/60 bg-rose-50/50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-950/40 text-xs font-medium transition-colors">
      ...
      {isLocationModalOpen && (<LocationPickerMap isModal={true} restaurantName={tenant.tradeName} initialLocation={tenant.location} onSaveLocation={(updatedLoc) => {
                if (onUpdateLocation) {
                    onUpdateLocation(tenant, updatedLoc);
                }
                setIsLocationModalOpen(false);
            }} onClose={() => setIsLocationModalOpen(false)} onOpenKeyConfig={onOpenKeyConfig}/>)}
    </AnimatePresence>);
};
