...
import { X, MapPin, Key, ExternalLink, Check, Sparkles, ShieldCheck, HelpCircle, Trash2, } from 'lucide-react';
import { getGoogleMapsApiKey, saveGoogleMapsApiKey, clearGoogleMapsApiKey } from '../../lib/maps';
import { useToast } from '../common/Toast';
export const GoogleMapsKeyModal = ({ isOpen, onClose, onKeySaved, }) => {
    ...
    const handleSave = (e) => {
        ...
        onKeySaved(cleanKey);
        showToast(cleanKey ? 'Google Maps Key Saved' : 'Google Maps Key Cleared', cleanKey
            ? 'Your Google Maps Platform key is active for Pakistan maps and geocoding.'
            : 'Reverted to local coordinates view.', cleanKey ? 'success' : 'info');
        onClose();
    };
    const handleClear = () => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/60 backdrop-blur-2xs p-4">
      <div className="relative w-full max-w-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-2xl overflow-hidden text-xs">
        ...
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-1 rounded text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
            <X className="w-4 h-4"/>
          </button>
        </div>
        ...
        <form onSubmit={handleSave} className="p-4 space-y-4">
          ...
          <div className="p-3 rounded-md bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20 space-y-2">
            ...
            <div className="pt-1 flex items-center gap-2">
              <a href="https://mapsplatform.google.com/maps-demo-key?utm_campaign=gmp_mcp_codeassist_v1_aistudio" target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded font-medium text-xs shadow-xs transition-colors">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input type="text" value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)} placeholder="AIzaSy... (Paste Google Maps API key or Demo Key here)" className="w-full pl-9 pr-3 py-2 bg-zinc-50 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-700 rounded-md font-mono text-xs text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"/>
            </div>
          ...
          <div className="pt-2 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-2">
            <div>
              {currentKey && (<button type="button" onClick={handleClear} className="flex items-center gap-1 px-2.5 py-1.5 text-red-600 hover:text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 rounded font-medium transition-colors">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={onClose} className="px-3 py-1.5 border border-zinc-200 dark:border-zinc-700 rounded font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                Cancel
              </button>
              <button type="submit" className="flex items-center gap-1.5 px-4 py-1.5 bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-900 rounded font-medium transition-colors shadow-xs">
