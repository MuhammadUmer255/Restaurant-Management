...
import { MapPin, Search, Crosshair, Building2, ExternalLink, PlusCircle, Key, } from 'lucide-react';
import { APIProvider, Map, AdvancedMarker, Pin, InfoWindow, } from '@vis.gl/react-google-maps';
import { PAKISTAN_CITIES, DEMO_MAP_ID, GMP_ATTRIBUTION_ID, getGoogleMapsApiKey, getGoogleMapsExternalUrl, } from '../../lib/maps';
import { StatusBadge } from '../common/Badge';
export const RestaurantFleetMap = ({ tenants, onSelectTenant, onImpersonate, onOpenOnboarding, onEditLocation, onOpenKeyConfig, }) => {
    ...
    const handleFocusTenant = (tenant) => {
    ...
    return (<div className="flex flex-col h-[calc(100vh-115px)] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg shadow-sm overflow-hidden text-xs">
      {/* Top Controls Bar */}
      <div className="p-3 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/90 flex flex-wrap items-center justify-between gap-2.5 shrink-0">
        ...
        {/* Action Buttons */}
        <div className="flex items-center gap-2">
          <button onClick={onOpenKeyConfig} className={`flex items-center gap-1 px-2.5 py-1.5 rounded-md font-medium text-xs transition-colors ${apiKey
          ...
          <button onClick={onOpenOnboarding} className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-900 rounded-md font-medium text-xs shadow-xs transition-colors">
      ...
      <div className="px-3 py-2 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex flex-wrap items-center justify-between gap-2 shrink-0">
        <div className="flex items-center gap-1.5 overflow-x-auto py-0.5 no-scrollbar">
          ...
          {['ALL', 'Islamabad', 'Lahore', 'Karachi', 'Rawalpindi', 'Peshawar', 'Faisalabad'].map((cityName) => (<button key={cityName} onClick={() => handleCitySelect(cityName)} className={`px-2.5 py-1 rounded-md text-[11px] font-medium whitespace-nowrap transition-colors ${selectedCity.toLowerCase() === cityName.toLowerCase()
        ...
        <div className="flex items-center gap-2">
          {/* Status Filter */}
          <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="px-2 py-1 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md text-xs text-zinc-800 dark:text-zinc-200">
          ...
          <div className="relative">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search restaurant or address..." className="w-48 sm:w-64 pl-8 pr-3 py-1 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md text-xs text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"/>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 min-h-0 overflow-hidden relative">
        ...
        <div className="lg:col-span-4 border-r border-zinc-200 dark:border-zinc-800 flex flex-col h-full bg-zinc-50/50 dark:bg-zinc-900/50 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-zinc-200 dark:divide-zinc-800">
            {mappedTenants.length === 0 ? (<div className="p-6 text-center text-zinc-400 space-y-2">
                <Building2 className="w-8 h-8 mx-auto stroke-1 text-zinc-400"/>
                <p>No restaurants found matching filter criteria.</p>
              </div>) : (mappedTenants.map((t) => {
            const isSelected = activeTenant?.id === t.id;
            return (<div key={t.id} onClick={() => handleFocusTenant(t)} className={`p-3 transition-colors cursor-pointer text-left ${isSelected
                    ...
                    {t.location && (<div className="mt-2 space-y-0.5 text-[11px] text-zinc-600 dark:text-zinc-400">
        ...
        <div className="lg:col-span-8 relative h-full min-h-[400px] bg-zinc-100 dark:bg-zinc-950">
          {apiKey ? (<APIProvider apiKey={apiKey} region="PK" language="en">
              <div className="w-full h-full relative" style={{ height: '100%' }}>
                <Map mapId={DEMO_MAP_ID} center={mapCenter} zoom={zoom} onCenterChanged={(e) => setMapCenter(e.detail.center)} onZoomChanged={(e) => setZoom(e.detail.zoom)} gestureHandling="greedy" disableDefaultUI={false} internalUsageAttributionIds={[GMP_ATTRIBUTION_ID]} style={{ width: '100%', height: '100%' }}>
                  {mappedTenants.map((t) => {
                if (!t.location?.coordinates)
                    return null;
                const isActive = activeTenant?.id === t.id;
                const pinColor = t.status === 'Active'
                    ? '#059669' // Emerald
                    : t.status === 'Trial'
                        ? '#d97706' // Amber
                        : '#e11d48'; // Rose
                return (<AdvancedMarker key={t.id} position={t.location.coordinates} title={`${t.tradeName} - ${t.location.city}`} onClick={() => setActiveTenant(t)}>
                        <Pin background={pinColor} glyphColor="#ffffff" borderColor="#0f172a" scale={isActive ? 1.25 : 1.0}/>
                      </AdvancedMarker>);
            })}

                  {/* InfoWindow for active clicked pin */}
                  {activeTenant && activeTenant.location && (<InfoWindow position={activeTenant.location.coordinates} onCloseClick={() => setActiveTenant(null)}>
                      <div className="p-1 max-w-xs text-zinc-900 text-xs">
                        <div className="flex items-center justify-between gap-2 border-b border-zinc-200 pb-1.5 mb-1.5">
                          ...
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${activeTenant.status === 'Active'
                    ...
                    : 'bg-amber-100 text-amber-800'}`}>
                        ...
                        <div className="pt-1.5 border-t border-zinc-100 flex items-center justify-between gap-2">
                          <button type="button" onClick={() => onSelectTenant(activeTenant)} className="px-2 py-1 bg-zinc-900 text-white rounded text-[10px] font-medium hover:bg-zinc-800">
                            Details
                          </button>
                          <button type="button" onClick={() => onImpersonate(activeTenant)} className="px-2 py-1 bg-emerald-600 text-white rounded text-[10px] font-medium hover:bg-emerald-500">
                            Impersonate
                          </button>
                          <a href={getGoogleMapsExternalUrl(activeTenant.location.coordinates, activeTenant.tradeName)} target="_blank" rel="noreferrer noopener" className="p-1 text-zinc-600 hover:text-zinc-900" title="Open in Google Maps">
                ...
                setMapCenter({ lat: 30.3753, lng: 69.3451 });
                setZoom(6);
            }} className="p-2 rounded-md bg-white/95 dark:bg-zinc-900/95 text-zinc-700 dark:text-zinc-200 shadow-md border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-xs font-medium flex items-center gap-1.5 transition-colors" title="Fit Pakistan Nationwide View">
        ...
        <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center space-y-4 bg-zinc-50/50 dark:bg-zinc-900/50">
              ...
              <div className="flex items-center gap-2 pt-2">
                <button type="button" onClick={onOpenKeyConfig} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-md font-medium text-xs shadow-xs transition-colors">
                  <Key className="w-4 h-4"/>
                  <span>Configure Google Maps Key</span>
                </button>
                <a href="https://mapsplatform.google.com/maps-demo-key?utm_campaign=gmp_mcp_codeassist_v1_aistudio" target="_blank" rel="noreferrer noopener" className="flex items-center gap-1.5 px-3 py-2 border border-zinc-200 dark:border-zinc-700 rounded-md font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-xs transition-colors">
