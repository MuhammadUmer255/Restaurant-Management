...
import { LogOut, ShieldAlert, Utensils, Monitor, DollarSign, Clock, CheckCircle2 } from 'lucide-react';
export const ImpersonationBanner = ({ tenant, onExit }) => {
    return (<div className="bg-amber-500 text-slate-950 px-4 py-2 text-xs font-medium shadow-md flex items-center justify-between sticky top-0 z-40 border-b border-amber-600">
      ...
      <button onClick={onExit} className="flex items-center gap-1.5 px-3 py-1 bg-slate-950 text-white rounded-md font-semibold hover:bg-slate-800 transition-colors shadow-sm shrink-0 text-xs">
        <LogOut className="w-3.5 h-3.5"/>
        <span>Exit Impersonation</span>
      </button>
    </div>);
};
export const ImpersonatedTenantDashboard = ({ tenant, onExit, }) => {
    ...
    return (<div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onExit} className="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs font-semibold rounded-lg hover:bg-slate-800 transition-colors">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {sampleOrders.map((order) => (<div key={order.id} className="py-3 flex items-center justify-between text-xs">
              <div>
