...
import { Activity, Server, Database, Wifi, Zap, RefreshCw, Globe, } from 'lucide-react';
export const SystemTelemetryView = ({ telemetry, tenants = [] }) => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleRefresh} className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 shadow-2xs" title="Force telemetry healthcheck">
      ...
      {/* Services Grid */}
      {telemetry.length === 0 ? (<div className="bg-white dark:bg-[#0f141f] rounded-2xl p-10 border border-dashed border-slate-200 dark:border-slate-800 text-center">
          ...
          {telemetry.map((svc) => (<div key={svc.id} className="bg-white dark:bg-[#0f141f] rounded-2xl p-5 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${svc.status === 'Operational'
