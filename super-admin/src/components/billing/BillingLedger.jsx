...
import { Receipt, Download, Search, CheckCircle2, Edit2, Save, RotateCcw, Sparkles, ExternalLink, ChevronRight, } from 'lucide-react';
import { useToast } from '../common/Toast';
export const BillingLedger = ({ transactions, plans, tenants = [], onUpdatePlanTier, onRetryInvoice, onSelectTenantById, }) => {
    ...
    const isPkrFleet = useMemo(() => tenants.some((t) => t.currency === 'PKR') || transactions.some((t) => t.currency === 'PKR'), [tenants, transactions]);
    ...
    const { totalAddonTerminals, addonMrr, addonUnitPrice } = useMemo(() => {
        const unitPrice = plans.find((p) => p.id.includes('pro') || p.id.includes('starter'))?.additionalTerminalPrice ||
            (isPkrFleet ? 3500 : 35);
        let extraCount = 0;
        tenants.forEach((t) => {
    ...
    const failedTransactions = useMemo(() => transactions.filter((t) => t.status === 'Failed'), [transactions]);
    const failedVolume = useMemo(() => failedTransactions.reduce((acc, t) => acc + t.amount, 0), [failedTransactions]);
    // Editing Plan Tier state
    const [editingPlanId, setEditingPlanId] = useState(null);
    const [editMonthlyPrice, setEditMonthlyPrice] = useState(0);
    const [editPosTerminals, setEditPosTerminals] = useState(0);
    const [editAddonPrice, setEditAddonPrice] = useState(0);
    const filteredTransactions = transactions.filter((txn) => {
        const matchesSearch = txn.tenantName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const handleSavePlan = (plan) => {
        ...
        showToast(`${plan.name} Tier updated successfully`, `Base price updated to $${editMonthlyPrice}/mo with ${editPosTerminals} POS terminals.`, 'success');
    };
    const handleDownloadInvoiceReceipt = (txn) => {
        const receiptText = `=====================================================
...
=====================================================`;
        ...
        showToast('Receipt Downloaded', `Official receipt for ${txn.invoiceNumber} saved.`, 'success');
    };
    const handleExportLedgerCSV = () => {
        const headers = 'Invoice Number,Tenant ID,Tenant Name,Amount,Currency,Payment Method,Status,Billing Period,Date\n';
        const rows = filteredTransactions
            .map((t) => `"${t.invoiceNumber}","${t.tenantId}","${t.tenantName}",${t.amount},"${t.currency}","${t.method}","${t.status}","${t.billingPeriod}","${t.date}"`)
            .join('\n');
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'Succeeded':
                return (<span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/50">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"/>
            Succeeded
          </span>);
            case 'Pending':
                return (<span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/50">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-500"/>
            Pending
          </span>);
            case 'Failed':
                return (<span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800/50">
            <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-ping"/>
            Failed
          </span>);
            case 'Refunded':
                return (<span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
            Refunded
          </span>);
        }
    };
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100 dark:bg-slate-800/80 text-xs">
          <button onClick={() => setActiveTab('transactions')} className={`px-3 py-1.5 rounded-lg font-semibold transition-all ${activeTab === 'transactions'
            ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 shadow-2xs'
            : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>
            Recent Transactions
          </button>
          <button onClick={() => setActiveTab('pricing-tiers')} className={`px-3 py-1.5 rounded-lg font-semibold transition-all ${activeTab === 'pricing-tiers'
            ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 shadow-2xs'
            : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        }} className="text-left bg-white dark:bg-[#0f141f] rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs hover:border-indigo-400 dark:hover:border-indigo-600 transition-all hover:shadow-xs cursor-pointer group" title="Click to view all transactions in the ledger">
        ...
        }} className="text-left bg-white dark:bg-[#0f141f] rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs hover:border-indigo-400 dark:hover:border-indigo-600 transition-all hover:shadow-xs cursor-pointer group" title="Click to configure subscription pricing tiers">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold text-slate-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 uppercase tracking-wider transition-colors">
        ...
        }} className="text-left bg-white dark:bg-[#0f141f] rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs hover:border-indigo-400 dark:hover:border-indigo-600 transition-all hover:shadow-xs cursor-pointer group" title="Click to manage POS terminal add-on pricing">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold text-slate-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 uppercase tracking-wider transition-colors">
              Add-on Terminal MRR
            </span>
        ...
        }} className="text-left bg-white dark:bg-[#0f141f] rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs hover:border-rose-400 dark:hover:border-rose-600 transition-all hover:shadow-xs cursor-pointer group" title="Click to filter by failed transactions for instant retry">
          <div className="flex items-center justify-between">
      ...
      {activeTab === 'transactions' && (<div className="space-y-4">
          ...
          <div className="bg-white dark:bg-[#0f141f] rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" placeholder="Search transactions by tenant, invoice #, or TXN ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-slate-400 font-medium">Status:</span>
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-200 font-medium focus:outline-none">
              ...
              <button onClick={handleExportLedgerCSV} className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 font-semibold flex items-center gap-1.5 transition-colors">
              ...
              <table className="w-full text-left text-xs text-slate-600 dark:text-slate-400">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                  {filteredTransactions.length === 0 ? (<tr>
                    ...
                    : 'No transactions match your current search or status filter.'}
                    ...
                    </tr>) : (filteredTransactions.map((txn) => (<tr key={txn.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                      <td className="py-3.5 px-4 whitespace-nowrap font-mono font-bold text-slate-800 dark:text-slate-200">
                        {txn.invoiceNumber}
                      </td>
                      <td className="py-3.5 px-3">
                        {onSelectTenantById ? (<button onClick={() => onSelectTenantById(txn.tenantId)} className="text-left group/t block hover:opacity-80 transition-opacity cursor-pointer" title="Click to inspect tenant details & billing status">
                      ...
                      <td className="py-3.5 pr-4 pl-2 text-right whitespace-nowrap">
                        {txn.status === 'Failed' ? (<button onClick={() => onRetryInvoice(txn)} className="px-2.5 py-1 rounded-lg bg-rose-600 hover:bg-rose-700 text-white font-semibold text-[11px] inline-flex items-center gap-1 transition-colors">
                            <RotateCcw className="w-3 h-3"/>
                            <span>Retry Charge</span>
                          </button>) : (<button onClick={() => handleDownloadInvoiceReceipt(txn)} className="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" title="Download official receipt">
      ...
      {activeTab === 'pricing-tiers' && (<div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {plans.map((plan) => {
                ...
                return (<div key={plan.id} className={`bg-white dark:bg-[#0f141f] rounded-2xl p-6 border shadow-2xs flex flex-col justify-between transition-all ${plan.popular
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      {plan.popular && (<span className="px-2 py-0.5 rounded text-[10px] font-bold bg-indigo-600 text-white uppercase">
                          Standard Default
                        </span>)}
                    </div>
                    ...
                    {isEditing ? (<div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 space-y-3 text-xs">
                        <div>
                          <label className="font-semibold text-slate-700 dark:text-slate-300 block mb-1">
                            Monthly Price ($ USD)
                          </label>
                          <input type="number" value={editMonthlyPrice} onChange={(e) => setEditMonthlyPrice(Number(e.target.value))} className="w-full px-3 py-1.5 rounded-lg border bg-white dark:bg-slate-950 font-bold font-mono text-sm"/>
                        </div>
                        ...
                        <div>
                          ...
                          <input type="number" value={editPosTerminals} onChange={(e) => setEditPosTerminals(Number(e.target.value))} className="w-full px-3 py-1.5 rounded-lg border bg-white dark:bg-slate-950 font-bold font-mono text-sm"/>
                        </div>
                        ...
                        <div>
                          ...
                          <input type="number" value={editAddonPrice} onChange={(e) => setEditAddonPrice(Number(e.target.value))} className="w-full px-3 py-1.5 rounded-lg border bg-white dark:bg-slate-950 font-bold font-mono text-sm"/>
                        </div>
                    ...
                    <div className="pt-2 space-y-2 border-t border-slate-100 dark:border-slate-800 text-xs">
                      {plan.features.map((feat, i) => (<div key={i} className="flex items-start gap-2 text-slate-600 dark:text-slate-400">
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    {isEditing ? (<div className="flex items-center gap-2 w-full">
                        <button onClick={() => setEditingPlanId(null)} className="flex-1 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold hover:bg-slate-100 dark:hover:bg-slate-800">
                          Cancel
                        </button>
                        <button onClick={() => handleSavePlan(plan)} className="flex-1 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold flex items-center justify-center gap-1">
                          <Save className="w-3.5 h-3.5"/>
                          <span>Save Changes</span>
                        </button>
                      </div>) : (<button onClick={() => handleStartEditPlan(plan)} className="w-full py-2 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors">
