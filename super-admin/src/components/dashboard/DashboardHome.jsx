...
import { Users, CreditCard, DollarSign, TrendingUp, AlertTriangle, ArrowUpRight, ArrowDownRight, CheckCircle2, Clock, ChevronRight, Activity, RefreshCw, Zap, Store, Plus, } from 'lucide-react';
import { Sparkline } from '../common/Sparkline';
import { StatusBadge, PlanBadge } from '../common/Badge';
import { useToast } from '../common/Toast';
export const DashboardHome = ({ kpis, tenants, alerts, telemetry, onSelectTenant, onNavigateToTenants, onNavigateToBilling, onNavigateToTelemetry, onOpenOnboarding, onOpenProvisioning, onResolveAlert, onRefreshMetrics, }) => {
    const { showToast } = useToast();
    const [timeframe, setTimeframe] = useState('12M');
    const [activeChartHoverIdx, setActiveChartHoverIdx] = useState(null);
    ...
    const handleRefresh = () => {
        ...
        setTimeout(() => {
            ...
            showToast('Telemetry Synchronized', 'Live fleet metrics, ingress latency, and active clusters updated.', 'success');
        }, 600);
    };
    ...
    const avgSla = useMemo(() => {
        if (effectiveTelemetry.length === 0)
            return '0.00';
        const total = effectiveTelemetry.reduce((acc, s) => acc + s.uptimePercentage, 0);
        return (total / effectiveTelemetry.length).toFixed(2);
    }, [effectiveTelemetry]);
    const operationalPops = useMemo(() => effectiveTelemetry.filter((s) => s.status === 'Operational').length, [effectiveTelemetry]);
    ...
    const actionRequiredAlerts = useMemo(() => {
        ...
        tenants
            .filter((t) => t.limits?.posTerminals &&
    ...
    const maxVal = Math.max(...displayedChartData.map((d) => Math.max(d.signups, d.cancellations, 1)), 2);
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <button onClick={handleRefresh} disabled={isRefreshing} className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 transition-colors shadow-2xs" title="Refresh fleet telemetry & metrics">
            <RefreshCw className={`w-4 h-4 ${isRefreshing ? 'animate-spin text-indigo-600' : ''}`}/>
          </button>
          {onOpenProvisioning && (
            <button onClick={onOpenProvisioning} className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs transition-colors" title="Provision Restaurant Business in Supabase table 'tenants'">
              <Store className="w-3.5 h-3.5"/>
              <span>Provision Tenant</span>
            </button>
          )}
          <button onClick={onOpenOnboarding} className="flex items-center gap-2 px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-semibold transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {kpis.map((kpi) => {
            ...
            return (<div key={kpi.id} onClick={handleCardClick} className="bg-white dark:bg-[#0f141f] rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs flex flex-col justify-between group hover:border-indigo-400 dark:hover:border-indigo-600 cursor-pointer transition-all hover:shadow-md" title={`Click to manage in ${kpi.id === 'mrr' || kpi.id === 'churn-rate'
                    ? 'Billing Ledger'
              ...
              <div>
                ...
                <div className="mt-1 flex items-center gap-1.5 text-xs">
                  <span className={`inline-flex items-center font-semibold text-[11px] ${kpi.isPositive
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/60 flex items-end justify-between">
                ...
                <Sparkline data={kpi.sparkline} height={28} width={90} isPositive={isChurnCard ? !kpi.change.startsWith('+') : kpi.isPositive}/>
              </div>
            </div>);
        })}
      </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#0f141f] rounded-2xl p-5 sm:p-6 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-800/60">
            ...
            <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100 dark:bg-slate-800/80 text-xs self-start">
              {['30D', '90D', '12M'].map((tf) => (<button key={tf} onClick={() => setTimeframe(tf)} className={`px-2.5 py-1 rounded-lg font-medium transition-all ${timeframe === tf
          ...
          <div className="mt-6">
            ...
            return (<div key={d.month + index} onMouseEnter={() => setActiveChartHoverIdx(index)} onMouseLeave={() => setActiveChartHoverIdx(null)} className="flex-1 flex flex-col items-center justify-end h-full group relative cursor-pointer">
                    {/* Hover Tooltip Card */}
                    {isHovered && (<div className="absolute -top-20 z-30 px-3.5 py-2 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs shadow-xl pointer-events-none min-w-[200px] border border-slate-700 dark:border-slate-200">
                        ...
                        <div className="space-y-0.5 text-[11px]">
                          ...
                          {d.onboardedNames.length > 0 && (<p className="text-[10px] text-indigo-300 dark:text-indigo-600 truncate font-medium">
                    ...
                    <div className="flex items-center justify-center mb-1.5 h-3.5">
                      {d.netExpansion > 0 ? (<span className="text-[9px] font-bold font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-1 rounded">
                          +{d.netExpansion}
                        </span>) : d.netExpansion < 0 ? (<span className="text-[9px] font-bold font-mono text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/60 px-1 rounded">
                          {d.netExpansion}
                        </span>) : (<span className="w-1 h-1 rounded-full bg-slate-200 dark:bg-slate-700"/>)}
                    </div>

                    {/* Dual grouped bars */}
                    <div className="w-full flex items-end justify-center gap-1">
                      {/* Signups bar */}
                      <div style={{ height: `${signupHeight}px` }} className={`w-full max-w-[14px] rounded-t-sm transition-all duration-200 ${d.signups > 0
                    ? isHovered
                        ? 'bg-indigo-500'
                        : 'bg-indigo-600 group-hover:bg-indigo-500'
                    : 'bg-slate-200 dark:bg-slate-800 opacity-40'}`}/>
                      {/* Cancellations bar */}
                      <div style={{ height: `${cancelHeight}px` }} className={`w-full max-w-[14px] rounded-t-sm transition-all duration-200 ${d.cancellations > 0
                    ? isHovered
                        ? 'bg-rose-400'
        ...
        <div className="bg-white dark:bg-[#0f141f] rounded-2xl p-5 sm:p-6 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800/60">
              ...
              <button onClick={onNavigateToTelemetry} className="text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1 cursor-pointer">
            ...
            <div className="mt-4 space-y-3">
              {effectiveTelemetry.slice(0, 4).map((svc) => (<div key={svc.id} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800 text-xs hover:border-slate-200 dark:hover:border-slate-700 transition-colors">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-semibold text-slate-800 dark:text-slate-200 truncate">
                      {svc.name}
                    </span>
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold shrink-0 ${svc.status === 'Operational'
          ...
          <button onClick={onNavigateToTelemetry} className="w-full mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between text-[11px] text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors group cursor-pointer" title={`Inspect all ${effectiveTelemetry.length} live cluster nodes in System Telemetry`}>
            <span className="flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-[#0f141f] rounded-2xl p-5 sm:p-6 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800/60">
            ...
            <button onClick={onNavigateToTenants} className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1 cursor-pointer">
          ...
          <div className="mt-4 divide-y divide-slate-100 dark:divide-slate-800/60">
            {recentOnboardings.length === 0 ? (<div className="py-8 text-center rounded-xl bg-slate-50/50 dark:bg-slate-900/30 border border-dashed border-slate-200 dark:border-slate-800 text-xs">
                ...
                <button onClick={onOpenOnboarding} className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white transition-colors cursor-pointer">
                  <Plus className="w-3.5 h-3.5"/>
                  <span>Onboard First Restaurant</span>
                </button>
              </div>) : (recentOnboardings.map((tenant) => (<div key={tenant.id} onClick={() => onSelectTenant(tenant)} className="py-3 flex items-center justify-between cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 px-2 rounded-xl transition-colors group">
        ...
        <div className="bg-white dark:bg-[#0f141f] rounded-2xl p-5 sm:p-6 border border-slate-200/80 dark:border-slate-800/80 shadow-2xs">
          ...
          <div className="mt-4 space-y-3">
            ...
            return (<div key={alert.id} className="p-3.5 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/60 text-xs flex flex-col justify-between gap-3 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                    <div className="flex items-start gap-2.5">
                      {alert.severity === 'critical' ? (<AlertTriangle className="w-4 h-4 text-rose-500 shrink-0 mt-0.5"/>) : (<Clock className="w-4 h-4 text-amber-500 shrink-0 mt-0.5"/>)}
